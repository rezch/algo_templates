...
#define timer(func, ...) { \
        ...
        ::high_resolution_clock::now();auto elapsed_ns_inner = std \
        ::chrono::duration_cast<std::chrono \
        ::duration<double>>(end_time_inner - start_time_inner);std \
        ::cout << std::fixed << std \
...
#define rng(var, to) \
        arng((var), 0, (to))
#define mst(list, size, value) std::memset(&(list), value, sizeof((list)[0]) * (size));
...
#define ret(...) write(__VA_ARGS__); return;
...
template <class T, class W> inline bool set_max(T& a, W b)
{ { if (a >= (T)b) return false; } { a = b; return true; } }
template <class T, class W> inline bool set_min(T& a, W b)
{ { if (a <= (T)b) return false; } { a = b; return true; } }

template <class T> inline T topbit(T x)
{ return (x == 0 ? -1 : 63 - __builtin_clzll(0LL + x)); } // 0 -> -1
template <class T> inline T lowbit(T x)
{ return (x == 0 ? -1 : __builtin_ctzll(0LL + x)); } // 0 -> -1
template <class T> inline T popcnt(T x)
{ return __builtin_popcountll(0LL + x); }
template <typename T, typename U> inline T ceil(T x, U y)
{ return (x > 0 ? (x + y - 1) / y : x / y); }
template <typename T, typename U> inline T floor(T x, U y)
...
template <class T, class S=char> inline void print(T& list, S splitter = ' ')
...
read(T&& ...args) { ((std::cin >> args), ...); }
#define rt(inner_type, ...) inner_type __VA_ARGS__; read(__VA_ARGS__);
inline void write_() { std::cout << std::endl; }
inline void write() { std::cout << std::endl; }
template <class T, class... U> constexpr inline void
write_(T&& head, U&&... tail) { std::cout << ' ' << head; write_(tail...);}
template <class T, class... U> constexpr inline void
write(T&& head, U&&... tail) { std::cout << head; write_(tail...); }
template <class T, class... U> constexpr inline void
set(int n, T&& value, U&& ...args) { ((args.assign(n, value)), ...); }
