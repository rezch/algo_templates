...
#define arange(var, from, to) \
        for (int var = from; var < to; var++)
#define range(var, to) \
        arange(var, 0, to)
#define iterate(var, list) \
        for (auto& var : list)
#define mrange(vara, varb, ind1, ind2) \
        range(vara, ind1) range(varb, ind2)
#define print(list, ostream) \
        iterate(var, list) { ostream << var << ' '; } ostream << std::endl;
#define input(list, istream) \
    iterate(var, list) { istream >> var; }
